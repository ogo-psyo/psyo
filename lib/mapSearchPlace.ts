export type MapSearchPlace = {
 ...
 sourceUrl:string;retrievedAt:string;pointIsCenter:true;dogAccess:'unknown'|'yes'|'no'|'leashed'|'designated';
};
...
export function isMapSearchPlace(value:unknown):value is MapSearchPlace {
 ...
 p.pointIsCenter===true&&['unknown','yes','no','leashed','designated'].includes(p.dogAccess||'')&&typeof p.sourceUrl==='string'&&p.sourceUrl.startsWith('https://www.openstreetmap.org/')&&typeof p.retrievedAt==='string'&&Number.isFinite(Date.parse(p.retrievedAt));
}
