...
export type MapOperation = 'place_discovery' | 'walking_route' | 'map_search' | 'route_save' | 'library_change' | 'meeting_preview' | 'meeting_send' | 'gav_publish' | 'gav_response';
export async function measuredMapOperation(operation: MapOperation, _request: Request, run: () => Promise<Response>): Promise<Response> {
    ...
    const base = { channel: 'pso.map-gav', eventId, operation, provider: ['place_discovery','map_search','walking_route'].includes(operation) ? 'osm' : 'pso', version: 'map-gav-20260907' };
