...
const profile = source('components/exact/ExactProfile.tsx');
...
const health = source('components/exact/ExactRecords.tsx');

requireText(layout, "import '@/components/exact/exact-interface.css'", 'source design code');
requireText(layout, "import '@/components/exact/exact-extensions.css'", 'same primitives for extended functionality');
rejectText(layout, "import './redesign.css'", 'retired visual layer');
requireText(layout, 'exact-prototype-20260915', 'approved design contract');
...
requireText(page, '<ExactConversation', 'assistant sheet');
requireText(source('components/exact/ExactConversation.tsx'), 'props.error', 'assistant in-context failure');
...
requireText(social, "useState<'live' | 'meet'>('meet')", 'Гав opens on the approved discovery screen');
...
requireText(source('lib/mapSession.ts'), 'pso.map.active-route.v3:${petId}', 'pet-scoped map storage');
requireText(map, 'routeSessionKey(petId)', 'workspace uses pet-scoped storage');
...
requireText(health, 'props.onSaveEdit(props.editingId)', 'observation edit lifecycle');
