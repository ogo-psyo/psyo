...
const tabs = ['today','passport','assistant','map','shop'];
...
for (const size of sizes) {
  for (const tab of tabs) {
    ...
    await page.getByRole('button', { name: tab === 'today' ? 'Сегодня' : tab === 'passport' ? 'Собака' : tab === 'assistant' ? 'Спросить' : tab === 'map' ? 'Карта' : 'Вещи' }).click({ force: true });
    await page.waitForTimeout(300);
    await page.screenshot({ path: `${outDir}/${size.name}-${tab}.png`, fullPage: true });
    const m = await page.evaluate(() => ({
      scrollWidth: document.documentElement.scrollWidth,
      innerWidth,
      bodyText: document.body.innerText.slice(0,300),
      overflow: [...document.querySelectorAll('body *')].map((el) => { const r=el.getBoundingClientRect(); return {tag:el.tagName, cls:String(el.className||''), text:(el.textContent||'').trim().slice(0,60), left:r.left, right:r.right, w:r.width}; }).filter(x => x.w>1 && (x.left<-1 || x.right>innerWidth+1)).slice(0,10)
    }));
    results.push({size:size.name, tab, m});
    await page.close();
  }
}
await browser.close();
