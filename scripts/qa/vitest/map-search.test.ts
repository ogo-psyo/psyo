...
it('name search uses bounds only as a bias and stable provider identity; missing coordinates never become zero',async()=>{
 ...
 const response=await GET(new Request('http://localhost/api/map/search?q=парк&lat=55.75&lng=37.61&bounds=55.70,37.5,55.8,37.7'));const body=await response.json();expect(body.results).toHaveLength(2);expect(body.results[0].id).toBe('osm-way-100');const url=new URL(fetch.mock.calls[0][0]);expect(url.searchParams.has('bounded')).toBe(false);expect(url.searchParams.get('viewbox')).toBe('37.5,55.8,37.7,55.7');
});
...
it('categories are local and dog admission is never inferred from category',async()=>{
 const fetch=vi.fn().mockResolvedValue(Response.json({elements:[{type:'node',id:1,lat:55.75,lon:37.61,tags:{name:'Кафе',dog:'yes'}},{type:'way',id:2,center:{lat:55.751,lon:37.612},tags:{name:'Неизвестно'}}]}));vi.stubGlobal('fetch',fetch);
 const r=await GET(new Request('http://localhost/api/map/search?category=cafe&lat=55.75&lng=37.61'));expect(r.status).toBe(200);expect((await r.json()).results.map((x:{dogAccess:string})=>x.dogAccess)).toEqual(['yes','unknown']);expect(String(fetch.mock.calls[0][1].body)).toContain('around%3A3000');
 expect((await GET(new Request('http://localhost/api/map/search?category=injected&lat=55&lng=37'))).status).toBe(400);
});
