...
const itemRoute = readFileSync(required[2], 'utf8');
const checkinRoute = readFileSync(required[3], 'utf8');
const migration = readFileSync(required[4], 'utf8');
...
for (const token of ['updateHabitForOwner', 'archiveHabitForOwner']) {
  assert.ok(service.includes(token), `HabitService lifecycle missing: ${token}`);
}
...
for (const token of ['getAppSessionFromRequest', 'getRequestAuth', "problem('AUTH_REQUIRED'"]) {
  ...
  assert.ok(itemRoute.includes(token), `habit item route missing: ${token}`);
}
for (const token of ['export async function PATCH', 'export async function DELETE']) assert.ok(itemRoute.includes(token), `habit item route missing: ${token}`);
for (const token of [
