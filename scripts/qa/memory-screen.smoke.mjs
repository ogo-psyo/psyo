...
for(const [engine,width] of [['chromium',390],['webkit',320]]){
 const t=await setup(engine,width),{page,ctx,pet,state}=t;let rows=[],readFail=true,writeFail=false,deletes=0,writes=0;
 try{
  ...
  await ctx.route('**/api/agent/memory**',async route=>{
   ...
   if(req.method()==='POST'){writes++;if(writeFail)return reply({error:'TEST_WRITE_FAILURE'},503);const item=rows.find(x=>x.memory_key===body.key);assert.ok(item);item.content=body.content;return reply({memory:item});}
  ...
  const screen=page.locator('[data-exact-view=memory]');await screen.getByRole('alert').waitFor();assert.equal(await screen.locator('.memory-welcome').count(),0);
  readFail=false;await screen.getByRole('button',{name:'Повторить',exact:true}).click();await screen.locator('.memory-welcome').waitFor();
  assert.equal(await screen.locator('textarea,input').count(),0);assert.equal(await screen.getByText('Добавить предпочтение').count(),0);
  ...
  const mutations=()=>state.requests.filter(r=>r.method!=='GET'&&/assistant|agent/.test(r.path)).length;
  const before=mutations();await screen.getByRole('button',{name:'Перейти в чат'}).click();
  const chat=page.locator('[data-exact-conversation]');await chat.waitFor();assert.equal(mutations(),before);assert.equal(writes,0);
  await chat.getByLabel('Сообщение помощнику').fill('Мой неотправленный вопрос');
  await chat.getByRole('button',{name:'Вернуться к вводу'}).click();await screen.waitFor({state:'visible'});
  await screen.getByRole('button',{name:'Перейти в чат'}).click();assert.equal(await chat.getByLabel('Сообщение помощнику').inputValue(),'Мой неотправленный вопрос');assert.equal(mutations(),before);
  // Simulate a confirmed memory saved by the existing chat, not by this CTA.
  rows=[{id:'m-one',memory_key:'walk_preferences',content:'Любим тихие прогулки',source_run_id:'r-one'},{id:'m-two',memory_key:'Игрушки',content:'Без пищалок'}];
  await chat.getByRole('button',{name:'Вернуться к вводу'}).click();await screen.getByText('Любим тихие прогулки',{exact:true}).waitFor();
  assert.equal(await screen.locator('textarea').count(),0);await page.screenshot({path:`${out}/${engine}-saved.png`});
  const first=screen.locator('.memory-item').first();await first.getByRole('button',{name:'Изменить'}).click();await first.getByLabel('Что учитывать').fill('Отменяемый текст');await first.getByRole('button',{name:'Отмена'}).click();assert.equal(writes,0);await first.getByText('Любим тихие прогулки',{exact:true}).waitFor();
  ...
  assert.equal(await page.evaluate(()=>window.__memoryBacks.size),1);
  await page.evaluate(()=>[...window.__memoryBacks][0]());await page.locator('[data-exact-view=profile]').waitFor({state:'visible'});
  ...
  await screen.getByRole('button',{name:'Забыть',exact:true}).click();await screen.locator('.memory-welcome').waitFor();assert.equal(rows.length,0);assert.equal(deletes,3);
  ...
  assert.deepEqual(t.errors,[]);results.push({engine,width,pass:true,scope:'mock API; empty/read-error, chat no-send/draft-preserved, return refresh, read/edit/cancel/save-error/retry, forget-error/retry/other-item-safe, motion'});
