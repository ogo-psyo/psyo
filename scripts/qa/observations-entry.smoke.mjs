...
for(const [engine,width] of [['chromium',390],['webkit',320],['chromium',1100]]){
 ...
 try{
  ...
  const capture=async name=>{await page.evaluate(()=>document.fonts.ready);await page.screenshot({path:`${out}/${engine}-${width}-${name}.png`});assert.ok(await page.locator('#pso-exact-content').evaluate(el=>el.scrollWidth<=el.clientWidth+1),'horizontal overflow');};
  await capture('empty');
