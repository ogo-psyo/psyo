...
try {
  for (const viewport of [{ width: 320, height: 720 }, { width: 390, height: 844 }]) {
    ...
    const geometry = await page.evaluate(() => {
      const topbar = document.querySelector('.woof-topbar')?.getBoundingClientRect();
      const filters = document.querySelector('.woof-live-filters')?.getBoundingClientRect();
      return { viewport: innerWidth, scrollWidth: document.documentElement.scrollWidth, topbar: topbar && { top: topbar.top, bottom: topbar.bottom }, filters: filters && { top: filters.top, bottom: filters.bottom } };
    });
    if (geometry.scrollWidth > geometry.viewport) throw new Error(`${viewport.width}px: live filters overflow horizontally`);
    if (!geometry.topbar || geometry.topbar.top < 0 || !geometry.filters || geometry.filters.top < geometry.topbar.bottom) throw new Error(`${viewport.width}px: live controls are clipped or overlap`);
