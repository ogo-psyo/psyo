...
import { energyOptions, friendlinessOptions, lifeStageOptions, neuteredOptions, parasiteOptions, playStyleOptions, sexOptions, sizeOptions, coatOptions, socialOptions, temperamentOptions, trainabilityOptions, vaccineOptions, type DogProfile } from '@/lib/data';
...
const groups: Array<{ title: string; fields: Field[] }> = [
  { title: 'Паспорт и внешность', fields: [
    {key:'dogName',label:'Имя'}, {key:'lifeStage',label:'Возрастная группа',options:lifeStageOptions}, {key:'sex',label:'Пол',options:sexOptions}, {key:'neutered',label:'Стерилизация',options:neuteredOptions}, {key:'weight',label:'Вес'}, {key:'size',label:'Размер',options:sizeOptions}, {key:'coatType',label:'Шерсть',options:coatOptions}, {key:'colorMarks',label:'Окрас и особые приметы'}, {key:'microchip',label:'Номер чипа'},
  ]},
...
export function ExactProfileFields({ draft, onChange }: { draft: DogProfile; onChange: (patch: Partial<DogProfile>) => void }) {
  return <>{groups.map(group => <details key={group.title}><summary>{group.title}</summary>{group.fields.map(({key,label,options,multiline}) => {
    ...
    return <div className="field" key={key}><label htmlFor={id}>{label}</label>{options ? <select id={id} value={value} onChange={event=>onChange({[key]:event.target.value})}>
      <option value="">Не указано</option>{value && !options.includes(value) && <option value={value}>{value}</option>}{options.map(option=><option key={option} value={option}>{option}</option>)}
