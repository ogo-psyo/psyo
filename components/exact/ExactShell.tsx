...
export function ExactNavigation({ active, onNavigate }: { active: string; onNavigate: (route: 'today' | 'map' | 'nearby' | 'all' | 'profile') => void }) {
  ...
  const section = view && ({home:'today',voice:'today',conversation:'today',observe:'today',edit:'today',record:'today',map:'map',walk:'map',gav:'nearby',connections:'nearby',acquaintance:'nearby',meeting:'nearby',all:'all',history:'all',library:'all',care:'all','care-edit':'all','care-create':'all',things:'all',thing:'all',profile:'profile',editprofile:'profile',memory:'profile',documents:'profile',document:'profile',identity:'profile',settings:'profile','public-card':'profile',habits:'profile',diary:'all','map-tools':'map','gav-tools':'nearby'} as Record<string,string>)[view];
