...
export function ProductionMapWorkspace({
  petId, guest, authHeaders, agentSelection, agentWalkSelection, onReturnToAssistant,
  ...
  const acceptAgentSelection=useEffectEvent(()=>{
    ...
    consumedAgentPlace.current=agentSelection.token;setAgentWalkPreview(null);
  ...
  const acceptAgentWalk=useEffectEvent(()=>{
    ...
    setAgentWalkPreview(agentWalkSelection.walk);setSelectedSearchPoint(null);setCandidate(null);setFolded(true);onModeChange('view');
  });
...
<div className="map-workspace-tools">      {(!routeFocused || routeFlow === 'planning') && <>
    ...
    <section className="production-map-canvas" aria-label={`Карта прогулок ${dogName}`}>
      <LiveMap
        ...
        routePoints={agentWalkPreview?agentWalkPreview.path:calculationState==='preview'&&walkResult?walkResult.path:routePoints}
        routeStops={agentWalkPreview?agentWalkPreview.snaps.map(s=>s.point):routeFlow==='planning'||routeFlow==='plan-review'?activeStops.map(s=>s.point):[]}
        routeStopIds={agentWalkPreview?agentWalkPreview.stops.map(s=>s.placeId):activeStops.map(s=>s.placeId)}
        routeGaps={agentWalkPreview?[]:pathGaps}
        onMapClick={agentWalkPreview?undefined:mode === 'risk' ? onMapClick : routeFlow === 'planning' && !folded ? (event) => setCandidate({point:[event.latlng.lng,event.latlng.lat]}) : mode==='view'&&!routeFocused ? (event)=>{setMapSelection(event.latlng);setSelectedSearchPoint(null);} : undefined}
        ...
        onSelectFeature={id=>{
          if(routeFocused||agentWalkPreview)return;
        ...
        fitDraftRoute={Boolean(agentWalkPreview) || routeFlow === 'record-review' || routeFlow === 'plan-review'}
...
<div className="map-work-area">
    ...
    {routeFocused ? <section className="production-route-controller" data-route-controller aria-label={routeTitle}>
      ...
      {(agentPlaces.length>0||Boolean(agentWalkSelection))&&onReturnToAssistant&&<button type="button" className="agent-map-return" onClick={onReturnToAssistant}><span aria-hidden="true">←</span> К разговору</button>}
