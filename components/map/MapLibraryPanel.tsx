...
import { collectionPoints, type SavedPlace } from '@/lib/mapLibrary';
...
export function MapLibraryPanel({ store, onChoose, onPlan, canPlan }: {
    ...
    onChoose: (place: SavedPlace) => void;
    onPlan: (points: number[][]) => void;
    canPlan: boolean;
}) {
  ...
  <ol className="map-library-places">{places.map((place, index) => <li key={place.id}>
   ...
   <button type="button" onClick={() => onChoose(place)}><b>{place.title}</b><span>{place.detail}</span></button>
  ...
  {selected.length > 0 && <button type="button" disabled={!canPlan || busy} onClick={() => onPlan(collectionPoints(library, collection.id, selected))}>Собрать прогулку · {selected.length}</button>}
