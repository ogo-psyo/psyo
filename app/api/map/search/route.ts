...
export async function GET(request:Request){return measuredMapOperation('map_search',request,async()=>{
 const url=new URL(request.url),result=await searchMapPlaces(url.searchParams.get('q')||'',{category:url.searchParams.get('category'),bounds:url.searchParams.get('bounds'),lat:url.searchParams.get('lat'),lng:url.searchParams.get('lng'),signal:request.signal});
