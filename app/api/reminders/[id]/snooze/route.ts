...
export async function POST(request: Request, ctx: Ctx) {
  ...
  const owned = await supabase.from('reminders').select('id, pets!inner(owner_id)').eq('id', id).eq('pets.owner_id', ownerId).single();
  if (owned.error) return NextResponse.json({ error: 'REMINDER_NOT_FOUND' }, { status: 404 });
  const { data, error } = await supabase.from('reminders').update({ status: 'snoozed', snoozed_until: snoozedUntil }).eq('id', id).select('*').single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  await supabase.from('reminder_events').insert({ reminder_id: id, event_type: 'snoozed', payload: { snoozedUntil } });
