...
export async function PATCH(request: Request, ctx: Ctx) {
  ...
  const owned = await ownedReminder(auth.supabase, auth.user.id, id);
  ...
  const { data, error } = await auth.supabase.from('reminders').update(patch).eq('id', id).select('*').single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  await auth.supabase.from('reminder_events').insert({ reminder_id: id, event_type: 'updated', payload: patch });
...
export async function DELETE(request: Request, ctx: Ctx) {
  ...
  const owned = await ownedReminder(auth.supabase, auth.user.id, id);
  if (owned.error) return NextResponse.json({ error: 'REMINDER_NOT_FOUND' }, { status: 404 });
  await auth.supabase.from('reminder_events').insert({ reminder_id: id, event_type: 'deleted', payload: {} });
