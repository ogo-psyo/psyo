...
export async function POST(request: Request, ctx: Ctx) {
  ...
  const { data, error } = await auth.supabase.from('reminders').update({ status: 'done', completed_at: now }).eq('id', id).select('*').single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  await auth.supabase.from('reminder_events').insert({ reminder_id: id, event_type: 'completed', payload: { completedAt: now } });
