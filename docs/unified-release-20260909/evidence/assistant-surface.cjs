...
(async()=>{
 ...
 for(const engine of ['chromium','webkit'])for(const width of [320,390]){
  ...
  try{
   ...
   await nav('things');
   const capture=page.locator('.thing-capture');await capture.getByLabel('Нужно купить',{exact:true}).fill('Сохранившийся ввод');
   ...
   assert.equal(await capture.getByLabel('Нужно купить',{exact:true}).inputValue(),'Сохранившийся ввод');
