  ...
  for (const viewport of [{ width: 320, height: 720 }, { width: 390, height: 844 }, { width: 1280, height: 900 }]) {
    ...
    await page.addInitScript(() => {
      navigator.mediaDevices.getUserMedia = async () => {
        ...
        output.stream.getAudioTracks()[0].addEventListener('ended', () => audioContext.close());
    ...
    await page.locator('.app-tabs [data-route=profile]').click();
    await page.getByRole('button', {name:'Добавить запись',exact:true}).click();
    const capture = page.locator('[data-surface=capture] .voice-observation-capture');
    ...
    await page.waitForTimeout(1000);await assertActivePhase('recording');
